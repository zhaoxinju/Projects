...
//    GPIO_SetupPinOptions(31, GPIO_OUTPUT, GPIO_PUSHPULL);
    ...
    GPIO_SetupPinMux(4, GPIO_MUX_CPU1, 6);        /*CAN*/
...
CANIntHandler(void)
{
    ...
    else if(ulStatus == 2)
    {
        ...
        // Get the received message
        //
//        extern void CANMessageRecv(uint32_t ui32Base, uint32_t ui32ObjID, tCANMsgObject *pMsgObject,
//                      bool bClrPendingInt);
//        CANMessageRecv(CANA_BASE, 2, &sRXCANMessage, true);
...
//    GPIO_SetupPinOptions(31, GPIO_OUTPUT, GPIO_PUSHPULL);
    ...
    *(unsigned long *)ucRXMsgData = 0;
    sRXCANMessage.ui32MsgID = 1;                      // CAN message ID - use 1
    sRXCANMessage.ui32MsgIDMask = 0;                  // no mask needed for TX
    sRXCANMessage.ui32Flags = MSG_OBJ_RX_INT_ENABLE | MSG_OBJ_EXTENDED_ID;  // enable interrupt on RX
    sRXCANMessage.ui32MsgLen = sizeof(ucRXMsgData);   // size of message is 8
    sRXCANMessage.pucMsgData = ucRXMsgData;           // ptr to message content

//    CANMessageSet(CANA_BASE, 2, &sRXCANMessage, MSG_OBJ_TYPE_RX);
    /*extern void
    CANMessageSetRecv(uint32_t ui32Base, uint32_t ui32ObjID, tCANMsgObject *pMsgObject,
                  tMsgObjType eMsgType);
    CANMessageSetRecv(CANA_BASE, 2, &sRXCANMessage, MSG_OBJ_TYPE_RX);*/
