...
#define CAN_BR          1000     /*kbps*/
#define CAN_BRP         (20000/CAN_BR - 1)
...
int16 CanTxBuf[16] = {0,1,2,3,4,5,6,7}, CanRxBuf[16];
CAN_MSG CanMsgTx, CanMsgRx;

/** **********************************************************
 * @brief   CAN初始化;消息1为发送，消息2为接收
 * @return  void
 ************************************************************/
void ConfigureCan(void)
{
    ...
    /*波特率设置1000kbps*/
    tmp = CanaRegs.CAN_CTL.all;
    CanaRegs.CAN_CTL.all |= 0x41;   /*禁止写保护*/
    CanaRegs.CAN_BTR.all = 0x3440; /*TSEG2 4, TSEG1 5, SJW 2, Divide 10*/
    CanaRegs.CAN_BTR.all |= (CAN_BRP & 0x3F) | ((CAN_BRP << 10) & 0xF0000);
    CanaRegs.CAN_CTL.all = tmp;     /*开启写保护*/

    /*中断信号使能*/
    CanaRegs.CAN_CTL.bit.IE0 = 1;
    CanaRegs.CAN_CTL.bit.SIE = 1;
    CanaRegs.CAN_CTL.bit.EIE = 1;
//    CanaRegs.CAN_CTL.bit.DAR = 1;   /*禁止重发*/

//    CanaRegs.CAN_CTL.bit.Test = 1;  /*DEBUG 内环*/
//    CanaRegs.CAN_TEST.all = 0x100;

    CanaRegs.CAN_CTL.bit.Init = 0;  /*使能CANA*/
    CanaRegs.CAN_GLB_INT_EN.bit.GLBINT0_EN = 1;
    /*消息初始化*/
    CanMsgTx.MsgNo = 1;
    CanMsgTx.MsgID = 2;
    CanMsgTx.MsgLen = 8;
    CanMsgTx.pMsgData = CanTxBuf;

    CanMsgRx.MsgNo = 2;
    CanMsgRx.MsgID = 1;
    CanMsgRx.MsgLen = 8;
    CanMsgRx.pMsgData = CanRxBuf;

    while(CanaRegs.CAN_IF1CMD.bit.Busy)
    {
        ;
    }
    *(volatile uint16_t *)(CANA_BASE + CAN_O_IF1CMD + 2) = 0xB3;
    CanaRegs.CAN_IF1MSK.all = 0;
    CanaRegs.CAN_IF1ARB.all = 0xC0000000 | (CanMsgRx.MsgID & 0x1FFFFFFF);
    CanaRegs.CAN_IF1MCTL.all = (CanMsgRx.MsgLen & 0xFF) | 0x480;
    CanaRegs.CAN_IF1CMD.bit.MSG_NUM = CanMsgRx.MsgNo;
 ...
 * @return  0:空闲 1:正忙
 **********************************************************************************************************************/
Uint16 blCanBusy(void)
{
    return CanaRegs.CAN_IF1CMD.bit.Busy;
}
 ...
 * @return  -1:失败   0:正常发送
 **********************************************************************************************************************/
int16 CanSendMsg(void)
{
    Uint16 i;
    int16 * pIfData = (int16 *)(CANA_BASE + CAN_O_IF1DATA);
    ...
    for(i = 0; i < CanMsgTx.MsgLen; i++)
    {
        __byte(pIfData ++, 0) = *(CanMsgTx.pMsgData + i) & 0xFF;
    }

    *(volatile uint16_t *)(CANA_BASE + CAN_O_IF1CMD + 2) = 0xB3;
    CanaRegs.CAN_IF1MSK.all = 0;
    CanaRegs.CAN_IF1ARB.all = 0xE0000000 | (CanMsgTx.MsgID & 0x1FFFFFFF);
    CanaRegs.CAN_IF1MCTL.all = (CanMsgTx.MsgLen & 0xFF) | 0x980;
    CanaRegs.CAN_IF1CMD.bit.MSG_NUM = CanMsgTx.MsgNo;

    return 0;
}
/** ********************************************************************************************************************
 * @brief   CAN接收消息
 * @return  -1:无消息   0:接收到消息
 **********************************************************************************************************************/
int16 CanRecvMsg(void)
{
    Uint16 i;
    int16 * pIfData = (int16 *)(CANA_BASE + CAN_O_IF2DATA);

    if(CanaRegs.CAN_INT.bit.INT0ID != CanMsgRx.MsgNo)
    {
        return -1;
    }

    CanaRegs.CAN_IF2CMD.all = 0x7F0000 | (CanaRegs.CAN_IF2CMD.all & 0xFFFF);
    CanaRegs.CAN_IF2CMD.bit.MSG_NUM = CanMsgRx.MsgNo;
    while(CanaRegs.CAN_IF2CMD.bit.Busy)
    {
        ;
    }

    if(CanaRegs.CAN_IF2MCTL.bit.NewDat)
    {
        CanMsgRx.MsgLen = CanaRegs.CAN_IF2MCTL.bit.DLC;
        for(i = 0; i < CanMsgRx.MsgLen; i++)
        {
             *(CanMsgRx.pMsgData + i) = __byte(pIfData ++, 0);
        }
    ...
    /*清除中断信号*/
    if(CanaRegs.CAN_IF1CMD.bit.Busy)
    {
...
//*****************************************************************************
void
CANMessageRecv(uint32_t ui32Base, uint32_t ui32ObjID, tCANMsgObject *pMsgObject,
              bool bClrPendingInt)
{
    uint32_t ui32CmdMaskReg;
    uint32_t ui32MaskReg;
